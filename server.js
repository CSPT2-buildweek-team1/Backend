...
server.get('/', (req, res)  =>  {
    db('room')
        .then(data  =>  {
            res.status(200).json({graph: data.map(item =>  {
                return {
...
const cb = (error, response, body)   =>  {
    ...
    if(!body.errors && body)    {
        ...
        let keys = Object.keys(graph[currentRoom].exits)
        let unsearchedRoom = false
        for(let i = 0; i < keys.length; i++)    {
            if(graph[currentRoom].exits[keys[i]] === "?") {
                unsearchedRoom = true
            }
        }
        if(stack.length === 0 && unsearchedRoom === false && searchStack.length === 0) {
            searchStack = dfs(currentRoom)
        ...
        if(searchStack.length > 0) {
            ...
            timeout(cooldown)
                    ...
                    db('room')
                        .where({room_id: graph[currentRoom].room_id})
                        .then(data  =>  {
                            if(data.length === 0)   {
                                db('room')
...
const predictMove = (dir, predict)  =>  {
    ...
    const dfs = (startRoom, previousRoom, visited)  =>  {
        const room = graph[startRoom]
        console.log(previousRoom)
        const keys = Object.keys(room.exits)
        let directions = []
        if(visited) {
            directions.concat(visited)
        }
        for(let i = 0; i < keys.length; i++)    {
            const exit = room.exits[keys[i]]
            console.log(room.exits)
            directions.push(keys[i])
            if(exit === "?") {
                return keys[i]
            }   else if(previousRoom !== exit && directions.includes(exit) === false){
                return [keys[i]].concat(dfs(exit, startRoom, directions))
            }   else {
                continue;
            }
        }
